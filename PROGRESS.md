...
| 2026-08-05 | 45 | ayaan-cover-broken-links | Wired the client's **own** published cover (`ayaan and little village light.webp` — *Ayaan and the Little Village Light*) across the site + fixed broken links from covers the client deleted. Broken-link audit (regex over app/components/lib, parens/spaces aware): only 3 referenced-but-missing files — `3d-book-cover (1).webp`, `(2).webp` (rendered on marketing + cover-design service heroes), and `wrap-2-img.webp` (dead testimonial `photo` data). Fixed: **all 5 service-hero `primary` covers → Ayaan** in `services/[slug]` + `[subslug]` (features our real title site-wide, kills the 2 broken); testimonial `bookCover (1)/(2)` → Ayaan, `photo` → `scene-reading-home.webp` (both fields are unused/dead but now valid). Corrected author name **Zunethida Ileahani → Ermelinda Hashani** (misread off the low-res photo; the clean cover reads "By Ermelinda Hashani, A Story of Courage and Kindness") and renamed `public/testimonials/ermelinda-hashani.{mp4,jpg}` + `lib/videoTestimonials.ts`. Client also deleted several IP-risk covers (Amazon-ID `81*`, `book1`, `violet and jo`, `showcase-cover (15).jpg`) — none referenced, deletions committed. `tsc` + `next build` clean (50 routes); re-audit shows 0 broken refs. ⚠️ Open: the spotlight **photo still shows the OLD typo cover in her hands** ("Zunethida"/"Cottrage") — swap `ermelinda-hashani.jpg` when the corrected photo arrives. Remaining genre/showcase covers may still be IP; `updated images/book covers/` (22 Kimberley covers) staged for a future swap. |
| 2026-08-05 | 44 | video-testimonial | New **Author Spotlight** section (video testimonials). Cinematic dark navy→teal band after the written testimonials on the homepage. Featured layout = editorial collage of the author's **photo holding her book** (still, 3:4) + a **vertical reel video player** (9:16), with a copy column (name / book / intro / CTAs). Custom player: `preload="none"` (9.6 MB clip only loads on click), branded play overlay, native controls after play. Future-proof: typed `lib/videoTestimonials.ts` array — first entry is featured, any extras auto-render as a compact video-card row (`.spotlight-more`). First author: Zunethida Ileahani, *Ayaan and the Little Village Light*. Media in `public/testimonials/zunethida-ileahani.{mp4,jpg}`. `tsc` + `next build` clean (50 routes), verified in built HTML. NOTE: `intro` is a truthful caption, not a verbatim quote — replace with her real words once transcribed. Video is a low-res phone reel (352×624); fine as-is but could be re-encoded smaller later. |
| 2026-08-05 | 43 | ios-footer-gap | Fixed an iPhone-only blank space below the footer. Cause: the "sticky footer reveal" pins `.site-footer` with `position:fixed;bottom:0`, which iOS Safari's dynamic toolbar mis-positions, leaving ~1 viewport of white below the footer (Android Chrome unaffected). Gated the whole fixed-footer reveal behind `@media (min-width:1025px) and (pointer:fine)` so no touch device (phone or tablet) gets it; they keep a normal in-flow footer. `next build` clean (50 routes). |
| 2026-08-05 | 42 | client-ux-fixes | Header glass bar gradient flipped (teal tint moved to the right end, left kept opaque) so the navy logo reads over dark heroes; removed CTA floating book images + restyled `PurpleCTA` to a centered band (`.index-wrap-4--centered`); dropped hero "UK · Worldwide" eyebrow (now optional prop); recoloured `.subservice-breadcrumb` light for the navy inner hero; fixed mobile horizontal scroll by clipping the off-canvas drawer (`overflow:hidden` on `.mobile-nav-root`) + pinned hamburger right (`.kdp-actions margin-left:auto` <992px); added 2 real client testimonials (Sid/Mike/illustrators) to `lib/content.ts`; set real FB (share link) + IG defaults in `Footer.tsx` and `schema.ts sameAs`; renamed top 3 packages → Starter / Professional / Premium Plus — Best Seller in `lib/packages.ts`. `tsc` clean; `next build` clean (49 routes). Not committed. NOTE: the 2 testimonial author names (Rebecca Coleman / Thomas Hartley) are placeholders — replace with the real names if available. |
